...
//        }
    ...
    private void loadTx() {
        ...
        if (!isTxDetail){
            ...
            txt_date.setText(simpleDateFormat.format(new Date(Long.parseLong(intent.getStringExtra("timereceived")))));
        }
        txt_transaction_id.setText(intent.getStringExtra("txid"));
        Coin amountValue = Coin.valueOf(new BigDecimal(intent.getStringExtra("amount")).longValue());
        txt_amount.setText(amountValue.toFriendlyString());
        Coin fee = null;
...
//        }
        ...
        if(intent.hasExtra("fee")) {
            fee = Coin.valueOf(new BigDecimal(intent.getStringExtra("fee")).longValue());
        }
...
//        txt_confirmations.setText(String.valueOf(transactionWrapper.getTransaction().getConfidence().getDepthInBlocks()));
        if (intent.hasExtra("confirmations"))
            txt_confirmations.setText(intent.getStringExtra("confirmations"));
